...
{
   if(3 > MutateFeedForwardEdge_host->number_of_nodes || 1 > MutateFeedForwardEdge_host->number_of_edges) return false;
...
void applyMutateFeedForwardEdge_mutateEdge(Morphism *morphism, bool record_changes)
{
   ...
   if(record_changes)
   {
      Edge *edge = getEdge(MutateFeedForwardEdge_host, host_edge_index);
      ...
      pushRemovedEdge(edge->label, edge->source, edge->target, edge->index,
                      edge->index < MutateFeedForwardEdge_host->edges.size - 1);
   }
   removeEdge(MutateFeedForwardEdge_host, host_edge_index);
   ...
   changeNodeMark(MutateFeedForwardEdge_host, host_node_index, 0);

   host_node_index = lookupNode(morphism, 1);
   HostLabel label_n1 = getNodeLabel(MutateFeedForwardEdge_host, host_node_index);
   if(record_changes) pushRemarkedNode(host_node_index, label_n1.mark);
   changeNodeMark(MutateFeedForwardEdge_host, host_node_index, 0);

   host_node_index = lookupNode(morphism, 2);
   HostLabel label_n2 = getNodeLabel(MutateFeedForwardEdge_host, host_node_index);
   if(record_changes) pushRemarkedNode(host_node_index, label_n2.mark);
   changeNodeMark(MutateFeedForwardEdge_host, host_node_index, 0);

   int source, target;
   int edge_array_size0 = MutateFeedForwardEdge_host->edges.size;
   ...
   host_edge_index = addEdge(MutateFeedForwardEdge_host, label, source, target);
   /* If the edge array size has not increased after the edge addition, then
      the edge was added to a hole in the array. */
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size0 == MutateFeedForwardEdge_host->edges.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, MutateFeedForwardEdge_host);
}
...
static bool fillpot_n2(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < MutateFeedForwardEdge_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "MutateFeedForwardEdge_mutateEdge", MutateFeedForwardEdge_host);
