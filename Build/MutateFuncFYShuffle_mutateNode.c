...
bool MutateFuncFYShuffle_b2 = true;

static bool evaluateCondition(void)
{
   return (MutateFuncFYShuffle_b2);
}

static void evaluatePredicateMutateFuncFYShuffle_2(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) MutateFuncFYShuffle_b2 = true;
   else MutateFuncFYShuffle_b2 = false;
}

static bool match_n1(Morphism *morphism);
static bool match_n0(Morphism *morphism);

bool matchMutateFuncFYShuffle_mutateNode(Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(MutateFuncFYShuffle_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicateMutateFuncFYShuffle_2(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               MutateFuncFYShuffle_b2 = true;
               break;
            }
         }
         else break;
...
void applyMutateFuncFYShuffle_mutateNode(Morphism *morphism, bool record_changes)
{
   Assignment var_0 = getAssignment(morphism, 0);
   string var_2 = getStringValue(morphism, 2);
   int var_3 = getIntegerValue(morphism, 3);
   int node_index = lookupNode(morphism, 0);
   int outdegree0 = getOutdegree(MutateFuncFYShuffle_host, node_index);
   ...
   int node_array_size2 = MutateFuncFYShuffle_host->nodes.size;
   int list_var_length1 = 0;
   ...
   host_node_index = addNode(MutateFuncFYShuffle_host, 1, label);
   /* If the node array size has not increased after the node addition, then
      the node was added to a hole in the array. */
   if(record_changes)
      pushAddedNode(host_node_index, node_array_size2 == MutateFuncFYShuffle_host->nodes.size);
   int node_array_size3 = MutateFuncFYShuffle_host->nodes.size;
   ...
   /* If the node array size has not increased after the node addition, then
      the node was added to a hole in the array. */
   if(record_changes)
      pushAddedNode(host_node_index, node_array_size3 == MutateFuncFYShuffle_host->nodes.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, MutateFuncFYShuffle_host);
}
...
bool fillpotMutateFuncFYShuffle_mutateNode(MorphismPot *pot, Morphism *morphism)
{
   if(2 > MutateFuncFYShuffle_host->number_of_nodes || 0 > MutateFuncFYShuffle_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < MutateFuncFYShuffle_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "MutateFuncFYShuffle_mutateNode", MutateFuncFYShuffle_host);
