...
bool MutateFuncFYShuffle_b0 = true;
bool MutateFuncFYShuffle_b1 = true;

static bool evaluateCondition(void)
{
   return (MutateFuncFYShuffle_b0 && MutateFuncFYShuffle_b1);
}

static void evaluatePredicateMutateFuncFYShuffle_0(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) MutateFuncFYShuffle_b0 = true;
   else MutateFuncFYShuffle_b0 = false;
}

static void evaluatePredicateMutateFuncFYShuffle_1(Morphism *morphism)
{
   Assignment assignment_1 = getAssignment(morphism, 1);
   /* If the variable is not yet assigned, return. */
   if(assignment_1.type == 'n') return;
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) MutateFuncFYShuffle_b1 = true;
   else MutateFuncFYShuffle_b1 = false;
}

static bool match_n0(Morphism *morphism);

bool matchMutateFuncFYShuffle_pickNode(Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < MutateFuncFYShuffle_host->nodes.size; host_index++)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicateMutateFuncFYShuffle_0(morphism);
            evaluatePredicateMutateFuncFYShuffle_1(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               MutateFuncFYShuffle_b0 = true;
               MutateFuncFYShuffle_b1 = true;
               break;
            }
         }
         else break;
...
void applyMutateFuncFYShuffle_pickNode(Morphism *morphism, bool record_changes)
{
   ...
   changeNodeMark(MutateFuncFYShuffle_host, host_node_index, 3);
...
bool fillpotMutateFuncFYShuffle_pickNode(MorphismPot *pot, Morphism *morphism)
{
   if(1 > MutateFuncFYShuffle_host->number_of_nodes || 0 > MutateFuncFYShuffle_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < MutateFuncFYShuffle_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "MutateFuncFYShuffle_pickNode", MutateFuncFYShuffle_host);
